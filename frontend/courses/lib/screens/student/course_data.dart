...
import 'package:courses/screens/student/profile_page.dart';
...
class SCourseData extends StatefulWidget {
  final Map<String, dynamic> data;
  final bool wasItBought;
  const SCourseData({super.key, required this.data, required this.wasItBought});

  @override
  State<SCourseData> createState() => _SCourseDataState();
}

class _SCourseDataState extends State<SCourseData>
  ...
  Future bookCourse() async {
    final data = {
      'method': 'book_course',
      'course_id': widget.data['course_id'],
  ...
  Future addNotification() async {
    final data = {
      ...
      'student_id': globals.student.id,
      'notification_content': "Purchase completed successfully"
    };
    var response = await CallApi().postData(data);
    print("${globals.student.id} ------------------");
    print("${response.statusCode} ======================");
      ...
      body: SafeArea(
        ...
        child: Stack(
          children: [
                      ...
                      children: [
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            MyText(
                              text:
                                  "${rate.toPrecision(1)} ($numberOfReviews ${lang['Reviews']})",
                              color: Colors.white,
            ...
            // button book or download
            Positioned(
              top: Dimensions.height(720),
              left: Dimensions.horizontal(100),
              right: Dimensions.horizontal(100),
              child: widget.wasItBought
                  ? ElevatedButton(
                      onPressed: () {
                        //   double _progress = 0;
                        ...
                        //         url:
                        //             "${globals.domain}/courses/upload/books/before/${book['book_to_course_path']}",
                        //         name: "${book['book_to_course_name']}_before",
                        ...
                        //     // downloadPDF(
                        //     //     "${globals.domain}/courses/upload/books/after/${book['book_to_course_path']}",
                        //     //     "${book['book_to_course_name']}_after");
                        //   }
                      },
                      child: Text("${globals.lang['Download Books']}"),
                    )
                  : ElevatedButton(
                      onPressed: () {
                        bookCourse();
